...
def main():
    ...
    JPG_PATH = '/home/oberon/vidz/dataset1399hd/images/'
    XML_PATH = '/home/oberon/vidz/dataset1399hd/xmls/'
    ...
    for ind in range(len(aug_list)):
        ...
        for j in range(to_be_affected):
            rand_name, desc = random.sample(imgs, 1)[0]
            rand_img = np.array(Image.open(JPG_PATH + rand_name))
            fname = rand_name[:-4]
            img_aug = f().augment_image(rand_img)
            new_name = fname + '_{}'.format(desc)
            Image.fromarray(np.uint8(img_aug)).save(
                JPG_PATH + new_name + '.jpg')
